...
```{r}
bedrock <- lm(Salary_log ~ FO_pct + G + A + GF + GA + plus_mins + TOI + iSF + Yrs_in_NHL + Ht + Wt + Grit + iHF + iHA + iBLK + PIM + GP, data = scaled_data)
summary(bedrock)

e     <- resid(bedrock)
Yhat  <- predict(bedrock)
s     <- summary(bedrock)$sigma
es    <- e/s
```
...
pairwise <- subset(scaled_data, select = c(FO_pct, G, A, GF, GA, plus_mins, TOI, iSF, Yrs_in_NHL, Ht, Wt, Grit, iHF, iHA, iBLK, PIM, GP))
...
``` {r}
library(glmnet)
X         <- model.matrix(Salary_log ~ FO_pct + G + A + GF + GA + plus_mins + TOI + iSF + Yrs_in_NHL + Ht + Wt + Grit + iHF + iHA + iBLK + PIM + GP -1, data = scaled_data)
...
```{r}
baseline <- lm(Salary_log ~ G + A + GF + plus_mins + TOI + iSF + Yrs_in_NHL + Wt + GP, data = scaled_data)
summary(baseline)
...
```{r}
library(glmnet)
X         <- model.matrix(Salary_log ~ G + A + GF + plus_mins + TOI + iSF + Yrs_in_NHL + Wt + GP -1, data = scaled_data)
Y         <- scaled_data$Salary_log
